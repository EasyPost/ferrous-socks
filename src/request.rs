...
async fn connect_bind(bind_addr: IpAddr, connect_addr: IpAddr, connect_port: u16, connect_timeout: Duration) -> Result<TcpStream, tokio::io::Error> {
...
async fn connect_one(addr: IpAddr, port: u16, config: &Config) -> Result<Connection, tokio::io::Error> {
                ...
                match (item.is_ipv4(), addr.is_ipv4()) {
                    (true, true) => { 
                        if *item == IpAddr::V4(Ipv4Addr::new(0, 0, 0, 0)) {
                            return Ok(Connection::Connected(TcpStream::connect((addr, port)).await?))
                        } else {
                            return Ok(Connection::Connected(connect_bind(*item, addr, port, timeout).await?))
                        }
                    },
                    (false, false) => {
...
impl Request {
    pub async fn connect(self, config: &Config) -> Result<Connection, tokio::io::Error> {
        let conn = match self.address {
            Address::IpAddr(i) => {
                connect_one(i, self.dport, config).await?
            }
            Address::DomainName(d) => {
                ...
                for addr in tokio::net::lookup_host(lookup.as_str()).await? {
                    match connect_one(addr.ip(), self.dport, config).await? {
                        ...
                        Connection::AddressNotSupported => { saw_not_supported = true; },
                        Connection::ConnectionNotAllowed => { saw_not_allowed = true; },
                        _ => { },
                    }
                }
                if saw_not_allowed {
